...
function startAnimation() {
  isAnimating = true;
  animationProgress = 0;
  totalBlocks = v1Blocks.length;
}

// increment how many blocks we show each frame
function updateAnimation() {
  animationProgress += 3; // how many blocks appear per frame

  if (animationProgress >= totalBlocks) {
    animationProgress = totalBlocks;
    isAnimating = false;
  }
}
...
function renderArt() {
  ...
  // V1 colored layer (only draw the visible part)
  const limit = Math.min(animationProgress, v1Blocks.length);
  for (let i = 0; i < limit; i++) {
    const block = v1Blocks[i];
...
function generateArt() {
  ...
  // sort V1 in a snake pattern so the animation travels line by line
  v1Blocks.sort((a, b) => {
