...
// new generateArt，draw two frames（V2 Base, V1 Up）
function generateArt() {
 ...
 // V2 grid
 const gridV2 = Array(rows).fill().map(function() {
 ...
 for (let y = 0, row = 0; y < sourceImage.height; y += SAMPLE_STEP, row++) {
  for (let x = 0, col = 0; x < sourceImage.width; x += SAMPLE_STEP, col++) {
   ...
   const bw = blockSize;
   const bh = blockSize;
   feltifyRect(artCanvas, bx, by, bw, bh, gridV2[row][col], 1.2); // V2 feltifyRect
  }
  }
 }
 ...
 for (let y = 0, row = 0; y < sourceImage.height; y += SAMPLE_STEP, row++) {
  for (let x = 0, col = 0; x < sourceImage.width; x += SAMPLE_STEP, col++) {
   ...
   const bw = blockSize;
   const bh = blockSize;
   feltifyRectV1(artCanvas, bx, by, bw, bh, gridV1[row][col], 1.2); // feltifyRectV1
  }
  }
 }
